...
interface TmdbApiService {
    ...
    suspend fun getTrendingMovies(@Query("api_key") k: String): TmdbMovieResponse

    @GET("movie/popular")
    suspend fun getPopularMovies(@Query("api_key") k: String): TmdbMovieResponse

    @GET("movie/top_rated")
    suspend fun getTopRatedMovies(@Query("api_key") k: String): TmdbMovieResponse

    @GET("trending/tv/week")
    suspend fun getTrendingShows(@Query("api_key") k: String): TmdbTvResponse

    @GET("tv/popular")
    suspend fun getPopularShows(@Query("api_key") k: String): TmdbTvResponse

    @GET("tv/top_rated")
    suspend fun getTopRatedShows(@Query("api_key") k: String): TmdbTvResponse
...
object ApiClient {
    // HARDCODED TMDB KEY
    const val TMDB_KEY = "0d5f6d8e07ab385be6c228b7950798bf"
